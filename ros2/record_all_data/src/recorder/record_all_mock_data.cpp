...
const std::string image_topic_name = "/camera";
const std::string imu_topic_name = "/imu";

class MockBagRecorder : public rclcpp::Node
{
public:
    MockBagRecorder()
        : Node("record_all_data")
    {
        ...
        image_subscription_ = create_subscription<sensor_msgs::msg::Image>(
            image_topic_name, 10, std::bind(&MockBagRecorder::image_topic_callback, this, _1));

        imu_subscription_ = create_subscription<sensor_msgs::msg::Imu>(
            imu_topic_name, 10, std::bind(&MockBagRecorder::imu_topic_callback, this, _1));
    }
...
private:
    void image_topic_callback(std::shared_ptr<rclcpp::SerializedMessage> msg) const
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Received: mock Image message");
    ...
    {
        rclcpp::Time time_stamp = this->now();
        RCLCPP_INFO(this->get_logger(), "Received: mock Imu message");
...
int main(int argc, char *argv[])
{
    ...
    rclcpp::spin(std::make_shared<MockBagRecorder>());
    rclcpp::shutdown();
    return 0;
}
